...
(define (remove-duplicates s)
    (cond
        ...
        ([member? (car s) (cdr s)]
            (remove-duplicates (cdr s))
        )
        (else
            (cons (car s) (remove-duplicates (cdr s)))
        )
    )
)

(define (union s1 s2)
    (union-helper (remove-duplicates s1) (remove-duplicates s2))
)
...
(define (union-helper s1 s2)
    (cond 
        ...
        ([member? (car s2) s1] 
            (union-helper s1 (cdr s2))
        )
        (else 
            (union-helper (append s1 (list (car s2))) (cdr s2))
        )
    )
)

(define (intersect s1 s2)
    (intersect-helper (remove-duplicates s1) (remove-duplicates s2))
)

(define (intersect-helper s1 s2)
    (cond 
        ([null? s1] 
            '()
        )
        ([member? (car s1) s2]
            (cons (car s1) (intersect-helper (cdr s1) s2))
        )
        (else
            (intersect-helper (cdr s1) s2)
        )
    )
)
